...
cir-tac: docker_check clangir-link-build
	...
	mkdir -p $(CIRTAC_DIR)/cir-llvm16
	cp /tmp/cir-tac-build/tools/cir-llvm16/cir-llvm16 $(CIRTAC_DIR)/cir-llvm16/cir-llvm16
...
test-se: docker_check build
	cd seqra-cir-sast && ./gradlew :test \
		--tests org.seqra.cir.sast.CirProjectAnalyzeTest
