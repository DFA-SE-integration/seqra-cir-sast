...
open class CIRLanguageManager(val cp: CIRClasspath) : LanguageManager {
    ...
    override fun getCallExpr(inst: CommonInst): CommonCallExpr? {
        cirDowncast<CIRInst>(inst)
        return when (inst) {
            is CIRCallOpInst, is CIRTryCallOpInst -> inst
            else -> null
        }
    }
    ...
    override fun getCalleeMethod(callExpr: CommonCallExpr): List<CIRFunction> {
        return when (callExpr) {
            is CIRCallOpInst, is CIRTryCallOpInst -> callExpr.resolveDirectCall(cp)
