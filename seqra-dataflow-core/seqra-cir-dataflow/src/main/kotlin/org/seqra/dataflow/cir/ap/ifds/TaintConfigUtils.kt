...
import org.seqra.dataflow.cir.ap.ifds.taint.PassActionEvaluator
import org.seqra.dataflow.cir.ap.ifds.taint.SourceActionEvaluator
...
object TaintConfigUtils {
    fun sinkRules(config: CIRTaintRulesProvider, method: CommonMethod, statement: CommonInst) =
        config.sinkRulesForMethod(method, statement)

    fun <T> applyEntryPointConfig(
        config: CIRTaintRulesProvider,
        method: CommonMethod,
    ...
    inline fun <T : TaintConfigurationItem> List<T>.applyRuleWithAssumptions(
        ...
        val conditionEvaluator = CIRFactAwareConditionEvaluator(conditionFactReaders)
        ...
        for (rule in this) {
            ...
            val conditionEvaluatorWithAssumptions = CIRFactAwareConditionEvaluator(assumptionReaders)
            ...
            for (cube in assumptionExprDnf) {
                val expr = CIRMarkAwareConditionExpr.And(cube.literals.toTypedArray())
