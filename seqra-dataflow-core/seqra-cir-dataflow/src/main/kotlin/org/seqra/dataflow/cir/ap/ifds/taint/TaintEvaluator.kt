...
import org.seqra.dataflow.cir.ap.ifds.CIRMarkAwareConditionExpr
...
interface FactAwareConditionEvaluator {
    fun evalWithAssumptionsCheck(condition: CIRMarkAwareConditionExpr): Boolean
    fun assumptionExpr(): CIRMarkAwareConditionExpr?
...
class TaintPassActionEvaluator(
    ...
    private val factTypeChecker: CIRFactTypeChecker,
    private val factReader: FinalFactReader,
    private val positionTypeResolver: PositionResolver<MLIRType?>,
...
class TaintCleanActionEvaluator {
    ...
    private fun removeAllFacts(
        ...
        if (from !is PositionAccess.Simple) {
            val filteredFact = fact.factAp.filterFact(removeSubtreeFilter(from.accessors()))
            return filteredFact?.let(fact::replaceFact)
        }
    ...
    private fun removeFinalFact(
        ...
        val factWithoutFinal = fact.factAp.filterFact(
            removeSubtreeFilter(from.accessors() + TaintMarkAccessor(markRestriction.name))
        ) ?: return null
...
class TaintPassActionPreconditionEvaluator(
    ...
    private val typeChecker: CIRFactTypeChecker,
    private val returnValueType: MLIRType?,
...
private fun MutableList<FinalFactAp>.hackResultArray(
    ...
    typeChecker: CIRFactTypeChecker,
    resultPositionType: MLIRType?,
) {
...
private fun PositionAccess.accessors(): List<Accessor> = buildList {
    var current = this@accessors
    while (current is PositionAccess.Complex) {
...
private fun removeSubtreeFilter(path: List<Accessor>): FactTypeChecker.FactApFilter {
    require(path.isNotEmpty()) { "Expected non-empty access path" }

    fun build(index: Int): FactTypeChecker.FactApFilter = object : FactTypeChecker.FactApFilter {
        override fun check(accessor: Accessor): FactTypeChecker.FilterResult {
            if (accessor != path[index]) return FactTypeChecker.FilterResult.Accept
            if (index == path.lastIndex) return FactTypeChecker.FilterResult.Reject
            return FactTypeChecker.FilterResult.FilterNext(build(index + 1))
        }
    }
